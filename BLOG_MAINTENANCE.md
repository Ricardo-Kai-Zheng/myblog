...
```
myblog/
  index.html          # 首页：卡片网?+ 分页?0?页）
  archive.html        # 归档页：列表形式 + 分页?0?页）
  about.html          # 关于?  projects.html       # 项目?  css/style.css       # 全局样式（像?Minecraft主题?  js/main.js          # 共享JS：图标绘制、主题切换、滚动、显示动?  js/pagination.js    # 通用分页组件 Pagination ?  js/scene.js         # Canvas 英雄场景（像素天?草地?  posts/
    posts.json        # 文章元数据（所有文章的索引?    YYYY-MM-DD-N.html # 单篇文章页面（N=自增ID?    *.md              # Markdown 源文件（可选，方便编辑?```

### 渲染管道

```
posts.json  -->  index.html（fetch + Pagination 渲染卡片?            -->  archive.html（fetch + Pagination 渲染列表?```
...
| 常量 | ?| 位置 |
|------|----|------|
| 导航图标 canvas | `width="30" height="30"` | 所?HTML ?`.nav-icon` |
...
| 导航栏宽?| `--nav-width: 88px` | `css/style.css` `:root` |
| 响应式断?| `767px` | `css/style.css` `@media` |
...
?`posts/` 下创?`YYYY-MM-DD-N.html`，命名规则：
- `YYYY-MM-DD` = 发文日期
- `N` = 全局自增序号，取 `posts.json` 中最?`id` + 1
...
?`posts/posts.json` 数组末尾追加一条记录。格式：
...
# 检查部署状?gh api repos/Ricardo-Kai-Zheng/myblog/pages/builds/latest --jq '{status, duration, error:.error.message}'
```
...
- `index.html` ?footer ?`<a href="mailto:...">`?处）
- `about.html` ?EMAIL ?+ footer?处）
- `archive.html` ?footer?处）
- `projects.html` ?footer?处）

### 首页/归档页的分页设置

修改每页显示条数?- `index.html`：搜?`new Pagination(posts, 10, ...)`，改 `10` 为目标?- `archive.html`：搜?`new Pagination(posts, 20, ...)`，改 `20` 为目标?
---
...
缩放比例调整：修?`--px`（网格步长）+ `P`（JS 像素常量? 逐个 CSS 值按比例换算?
---

## 主题系统

- 存储键：`localStorage` 中的 `blog-theme`（?`"dark"` ?`"light"`?- 切换按钮：`#theme-toggle`，位于导航栏
- 图标：通过 `main.js` ?`drawIcon()` ?canvas 上绘制像素太?月亮
- 深色模式颜色：由 `applyTheme()` 动态设?CSS 变量（天空、草地、木头、文字等?- 系统偏好：首次访问遵?`prefers-color-scheme`，用户手动选择后覆?
---

## 像素图标系统

所有图标通过 `main.js` 中的 `drawIcon(canvas, iconType)` 函数?canvas 上逐像素绘制?
支持的图标类型（`data-icon` 属性值）?- `home` ?像素小屋
- `book` ?像素?- `pickaxe` ?像素?- `player` ?像素头像
- `paper` ?像素卷轴（文章卡片用?- `sun` ?8射线日轮（亮色模式）
- `moon` ?新月（暗色模式）
- `github` ?GitHub 猫（未使用，保留?- `mail` ?信封（未使用，保留）

添加新图标：?`main.js` ?`sprites` 对象中新增一个函数，?`px(x, y, color)` 按坐标画像素块。画布为 6×6 网格，每个像素块 `P×P`（当?`5×5`）?
---

## 部署

- 平台：GitHub Pages
- 方式：分支部署，源分?`master`，目?`/ (root)`
- 自动部署：每?push ?master 后GitHub Pages 自动构建
- 手动触发?  ```bash
  gh api repos/Ricardo-Kai-Zheng/myblog/pages/builds -X POST
  ```
- 检查状态：
  ...
  ```
- 构建时间：通常 60-90 ?- 网站地址：`https://ricardo-kai-zheng.github.io/myblog/`
...
**症状**：push 后收到构建错误邮件，状态显?`"errored"`

**常见原因**?1. **文件编码问题** ?`.md` 文件必须 UTF-8 编码，不能含 GBK/GB2312 字符。Jekyll 对非?UTF-8 字节零容忍?   - 修复：用 VS Code 右下角点击编??"Save with Encoding" ?UTF-8
2. **YAML front matter 语法错误**（如?`_config.yml`?
**查看错误详情**?```bash
...
### 文章不显?
**症状**：首?归档页看不到新文?
**排查顺序**?1. 确认 `posts/posts.json` 已更新且格式正确（用浏览器直接访?`https://ricardo-kai-zheng.github.io/myblog/posts/posts.json`?2. **不能?curl/Invoke-WebRequest 测试** ?文章由客户端 JS 动态渲染，这些工具不执?JavaScript。请用浏览器打开页面查看?3. 浏览器缓存：`Ctrl + Shift + R` 硬刷?4. GitHub Pages 部署延迟：检查构建状态是?`"built"`
5. 检?`filePath` 是否正确：相对于仓库根的路径，如 `posts/2026-05-19-1.html`

### 图标不显示或显示错误

**症状**：导航栏/主题切换图标空白或显示旧图标

**原因**?1. Canvas 尺寸?`P` 步长不匹??nav-icon 应为 `30×30`（P=5 × 6格），card-icon 应为 `43×43`
2. `main.js` ?`sprites` 对象缺少对应?`data-icon` 名称
3. 主题切换图标?`applyTheme()` 重新绘制，如?`#theme-toggle` 内无 `<canvas>` 则不会更?
### 样式错乱

**常见问题**?- 修改?`style.css` 中任一 font-size 但忘记同步修?`--px` ?JS 中的 `P` 常量
- 导航栏大小改?CSS ?`width` 但没?`--nav-width`，导?body padding 对不?- 响应式样式只改了桌面端忘了改 `@media (max-width:767px)` 内的